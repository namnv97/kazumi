				...
				<div class="col-md-3 col-xs-12 col-sm-12">
					<div class="lion-loyalty-panel-sidebar">
						...
						<ul class="nav nav-tabs">
    						...
    						<li><a data-toggle="tab" href="#earn-points">Kiếm điểm</a></li>
    						<li><a data-toggle="tab" href="#get-rewards">Đổi thưởng</a></li>
    						<li><a data-toggle="tab" href="#refer-friends">Giới thiệu</a></li>
    						<li><a data-toggle="tab" href="#graded">Hạng</a></li>
    						<li><a data-toggle="tab" href="#account">Tài khoản</a></li>
    						<li><a data-toggle="tab" href="#help">Trợ giúp</a></li>
  						</ul>
					</div>
				</div>
				<div class="col-md-9 col-xs-12 col-sm-12">
					<div class="tab-content">
					    ...
					    <div id="earn-points" class="tab-pane fade">
					      	<div class="lion-loyalty-panel-content">
					      		<div class="lion-loyalty-panel-content__header">Kiếm điểm thưởng</div>
					      		<div class="row">

					      			@foreach($earn_point as $key => $value)
						      		<div class="col-md-6 col-sm-6 col-xs-12">
						      			<div class="lion-loyalty-panel-rule-item" data-toggle="modal" data-target="#myModal-account" data-id="{{$value->id}}">
						      				<div class="lion-loyalty-panel-rule-item__icon" style="background: url({{$value->image}}) center/30px no-repeat #fff;"></div>
						      				<div class="lion-loyalty-panel-rule-item__title">{{$value->title}}</div>
						      				<div class="lion-loyalty-panel-rule-item__points"><span data-i18n-key="ui.dashboard.earn_points.points_per"><span class="value">{{number_format($value->point)}}</span> <span class="text">điểm <span class="lion-currency"><span class="lion-currency__value"> {{number_format($value->price)?number_format($value->price):"".' '.$value->unit}}</span></span></span></span></div>
						      			</div>
						      		</div>
						      		@endforeach
						      		...
						      		<div class="col-md-6 col-sm-6 col-xs-12">
						      			<div class="lion-loyalty-panel-rule-item">
						      				<div class="lion-loyalty-panel-reward-item__content">
						      					<div class="lion-loyalty-panel-reward-item__title">{{$value->name .' '.number_format($value->reward) .' '.$value->unit}}</div>
						      					<div class="lion-loyalty-panel-reward-item__meta">
						      						<span class="lion-loyalty-panel-reward-item__cost">
						      							<span>
						      								<span>
						      									<span class="value">{{number_format($value->point)}}</span> 
						      					...
						      					@if(Auth::user()->point_reward >= $value->point)
							      					<div class="ninh">
							      					<div class="lion-action-button lion-loyalty-panel-reward-item__redeem-button lion-loyalty-panel-reward-item__redeem-button--disabled">
							      						<span class="lion-loyalty-panel-reward-item__redeem-button-text">Lấy quà</span>
							      						<input type="hidden" name="idreward" value="{{$value->id}}">
							      					</div>
							      					</div>
						      					@else
					      							<div class="lion-action-button lion-loyalty-panel-reward-item__redeem-button lion-loyalty-panel-reward-item__redeem-button--disabled">
						      							<span class="lion-loyalty-panel-reward-item__redeem-button-text">Chưa đủ điểm</span>
					      							</div>
					    ...
					    <div id="account" class="tab-pane fade">
					      	<div class="lion-loyalty-panel-content">
					      		<div class="lion-loyalty-panel-content__header">Hoạt động gần đây</div>
					      		<div class="lion-loyalty-panel-page-history">
					      			<table class="lion-customer-history-table table">
					      				<thead>
					      					<tr>
					      						<th data-i18n-key="ui.general.date" class="lion-customer-history-table__header-cell">Thời gian</th>
					      				...
					      				<tbody>
					      					...
					      					<tr class="lion-customer-history-table__row">
					      						<td class="lion-customer-history-table__row-cell lion-customer-history-table__row-date">{{date('d-m-Y',strtotime($value->created_at))}}</td>
					      						<td data-i18n-key="ui.general.activity" class="lion-customer-history-table__row-cell">Đã kích hoạt</td>
					      						<td class="lion-customer-history-table__row-cell">{{$value->action}}</td>
					      						<td class="lion-customer-history-table__row-cell">{{$value->point}}</td>
					      						<td class="lion-customer-history-table__row-cell lion-customer-history-table__row-status">
					      							<div class="lion-customer-history-table__bubble lion-history-state-bubble lion-history-state-bubble--approved">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$('.ninh').click(function(){
				...
				$.ajax({
					url: "{{route('client.account.getreward')}}",
					method: 'post',
					dataType: 'json',
					data: {
                        id: id,
			...
			$('body').on('show.bs.modal','#myModal-account' , function(e){


				let id = $(e.relatedTarget).data('id');

				$.ajax({
					url : "{{route('client.account.modelearn')}}",
					method: 'post',
					dataType: 'json',
					data : {
						id: id,
					...
					let html = `<div class="modal-content">
				      <div class="modal-header">
				        <button type="button" class="close" data-dismiss="modal">&times;</button>
				        <h4 class="modal-title">${result.data.title}</h4>
				      </div>
				      <div class="modal-body">
				        	<p>Tặng ${result.data.point} điểm mỗi khi bạn ${result.data.title} ${(result.data.unit)? "mỗi khi bạn mua hàng trị giá "+result.data.price + ' ' + result.data.unit : ''} </p>
